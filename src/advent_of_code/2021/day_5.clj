...
(defn data-line-to-coordinate-map [string]
  (for [number (str/split string #" -> |,")]
    (utils/parse-int number)))

(defn horizontal-ends-to-line [coordinates]
  (let [[x-min x-max] (sort [(nth coordinates 0) (nth coordinates 2)])]
    (for [x (range x-min (+ x-max 1))]
      [x (nth coordinates 1)])))

(defn vertical-ends-to-line [coordinates]
  (let [[y-min y-max] (sort [(nth coordinates 1) (nth coordinates 3)])]
    (for [y (range y-min (+ y-max 1))]
      [(nth coordinates 0) y])))

(defn join-coordinates
  [x-values y-values]
  (for [n (range (count x-values))]
    [(nth x-values n) (nth y-values n)]))

(defn diagonal-ends-to-line [coordinates]
...
(defn end-coordinates-to-all-point-horizontal-and-vertical-only [coordinate-sets]
...
(defn end-coordinates-to-all-point [coordinate-sets]
  (apply concat (for [coordinates coordinate-sets]
                  (cond
                    (= (nth coordinates 1) (nth coordinates 3)) (horizontal-ends-to-line coordinates)
                    (= (nth coordinates 0) (nth coordinates 2)) (vertical-ends-to-line coordinates)
...
(defn day-five-part-two [file-name]
  (count (filter #(< 1 (val %))
                 (frequencies
