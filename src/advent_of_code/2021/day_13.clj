...
(defn fold-y [dots line]
  (distinct (for [dot dots]
              {:x (:x dot)
               :y (- line (Math/abs (- line (:y dot) )))})))

(defn fold-x [dots line]
  (distinct (for [dot dots]
              {:x (- line (Math/abs (- line (:x dot))))
               :y (:y dot)})))

(defn day-13 [dots-file folds-file]
...
(defn print-matrix [list-of-points]
...
(defn day-13-part-two [dots-file folds-file]
  (let [dots (utils/read-file-to-coordinates dots-file)
